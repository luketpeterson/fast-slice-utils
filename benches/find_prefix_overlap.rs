...
// // Adam: Performance on x86 is poor, but the core::simd version is on par with AVX2
// #[inline(always)]
...
/// The reference version of `find_prefix_overlap` on the `short_prefix_zipf_16` test cases
#[divan::bench()]
fn short_prefix_zipf_16_reference(bencher: Bencher) {
    let pairs = make_zipf_keys(NUM_KEYS, 16, 42, 1.5);
    ...
    bencher.bench_local(|| {
        pairs.iter().for_each(|(_, l, r)| {
            let result = count_shared_reference(l, r);
            std::hint::black_box(result);
        });
    });
}
...
// =======================================================================================
// Everything below this was cribbed drectly from [`pathmap::fuzzer`]
//
// Eventually `fuzzer` will become its own crate, and this code (and also pathmap) can just
// link it.  But for now I'm just including the parts we need for this benchmark.
// =======================================================================================
